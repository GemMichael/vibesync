...
export default function Navbar() {
    ...
    const fetchSearchResults = async (searchQuery) => {
        ...
        try {
            const response = await axios.get(`http://localhost:5000/api/auth/search?query=${searchQuery}`);
        ...
        <nav className="fixed top-0 z-50 w-full px-6 xl:px-0 bg-white shadow-md">
            <div className="flex items-center justify-between max-w-[1160px] w-full py-5 mx-auto">
                ...
                <div className="flex gap-8 text-black">
                    <Home className="w-6 h-6 cursor-pointer" onClick={() => router.push("/")} />
                    <User className="w-6 h-6 cursor-pointer" onClick={() => router.push("/Profile")} />
                    <LogOut 
