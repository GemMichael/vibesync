...
export default function ProfilePage() {
  ...
  if (loadingUser) return <p className="text-center text-lg font-semibold">Loading profile...</p>;

  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center pt-10 mt-20">
      {/* Profile Header */}
      <div className="flex items-center space-x-4">
        <img src="https://i.pravatar.cc/100" alt="Profile" className="w-24 h-24 rounded-full border-4 border-gray-300" />
        <div>
              ...
              posts.map((post) => (
                <div key={post._id} className="bg-gray-200 shadow-lg rounded-3xl p-4">
                  ...
                  <div className="flex justify-between mt-3 text-sm font-semibold italic">
                    <span className="cursor-pointer underline">Feel it! ({post.likes.length})</span>
                    <span className="cursor-pointer underline">Add your vibe</span>
                  </div>
                </div>
              ))
            ) : (
              <p className="text-center text-gray-500 mt-4">No vibes posted yet.</p>
            )}
          </div>
        </div>
      </div>
